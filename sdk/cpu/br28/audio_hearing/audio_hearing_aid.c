...
#define TCFG_AUDIO_HEARING_AID_ENABLE 1 // liurui

// #define TCFG_USER_TWS_ENABLE 1
// #define GEN_SINE_PCM 1
...
#ifdef REALHEAR_ENABLE
#include "../realhear/real_hear.h"
static void *realhear = NULL;
#else
void real_wdrc_volume_set(int mode)
{
    // api testing
}
#endif
...
// #define USE_CPULOAD_PRINT 1
//  160 short input, rtc ns wdrc 16% cpuload (test jl devboard 20240911)
#ifdef USE_CPULOAD_PRINT
// cpu load :240807
// config_system_info = 1;           // lib_system_config.c
void os_system_info_output(void); // 输出信息，可以定时器5秒调用一次
void os_system_info_reset(void);  // 重置运行时间

u16 cpu_print_timer;
static void cpu_load_schedule(void *priv)
{
    // printf("[DHA]schedule,cur_state = %s\n", dha_state_str[dha_schedule.state]);
    // sys_timer_del(cpu_print_timer);
    // cpu_print_timer = 0;
...
// 生成正弦波函数
// void generateSineWave(int16_t *buffer, int length,
// double frequency, double dB_SPL, int sampleRate)
// {
//     double amplitude = pow(10, dB_SPL / 20.0) * 32767.0 / 120.0; // 计算振幅
...
// }
void generateSineWave(int16_t *buffer, int length,
                      float frequency, float dB_SPL, int sampleRate)
{
    float amplitude = powf(10, dB_SPL / 20.0) * 32767.0 / 120.0; // 计算振幅
...
// int main() {
//     int length = 128; // 可以设置为64, 128, 160
//     double frequency = 1000.0; // 频率，单位Hz
//     double dB_SPL = 90.0; // 声压级，单位dB
//     int sampleRate = 16000; // 采样率，单位Hz

//     int16_t buffer[length];
//     generateSineWave(buffer, length, frequency, dB_SPL, sampleRate);
...
static float notch_mem[2] = {0};
static float notch_radius = 0.984f; // 0.92f; //0.982f; // 0.96f;//0.97f; //0.982f;
static inline void filter_dc_notch16(const short *in, short *out, int len)
{
    int i;
    float den2;
    // den2 = radius * radius + .7 * (1 - radius) * (1 - radius);
    den2 = notch_radius * notch_radius + 0.7 * (1.0 - notch_radius) * (1.0 - notch_radius); // 0.964550793
    // den2 = 0.964550793;
    for (i = 0; i < len; i++)
    {
        float vin = in[i];
        float vout = notch_mem[0] + vin;

        float tmpout = notch_radius * vout;

        notch_mem[0] = notch_mem[1] + 2 * (-vin + tmpout);
        notch_mem[1] = vin - (den2 * vout);
        out[i] = SATURATE32((tmpout), 32767);
    }
}
#endif

#ifdef REAL_DUMP_AUDIO_PCM_DATA
#include "debug.h"
#include "../realhear/real_ringbuf.h"

static rbuf_t *ringbuf_dump;
static int save_cnt = 0;
static int dump_is_running = 1;
static OS_SEM real_dump_sem;
void dump_file_thread()
{
    static short dump_tmp[64] = {0};
    while (dump_is_running == 1)
    {
        // 阻塞等待数据
        os_sem_pend(&real_dump_sem, 0);

        if (rbuf_used(ringbuf_dump) >= 128)
        {
            rbuf_read(ringbuf_dump, (char *)dump_tmp, 128);
            // for (int i = 0; i < 64; ++i)
            // {
            //     short pcm1 = dump_tmp[i];
            //     printf("%d, ", pcm1);
            // }
            printf_buf(dump_tmp, 128);
        }
        // else{
        //     //const TickType_t xDelay = 5 / portTICK_PERIOD_MS; // 8 10
        //     //vTaskDelay(xDelay);
        //     for (int i = 0; i < 64; ++i)
        //         dump_tmp[i] = 0;
        // }
...
//*********************************************************************************//
/*mic声音淡入使能*/
#define DHA_FADEIN_ENABLE 0 // 1

/*均衡器*/
#define DHA_EQ_ENABLE 1 // 1 //0 // 1

/*均衡器2*/
#define DHA_EQ2_ENABLE 0

/*动态范围控制*/
#define DHA_DRC_ENABLE 0 // 1

/*[啸叫抑制-移频法]Howling Suppress - FreqShift*/
#define DHA_HS_FS_ENABLE 0 // 1
...
#define HEARING_AID_CLK (160 * 1000000L)
#define DHA_MIC_SAMPLE_RATE TCFG_AUDIO_DHA_MIC_SAMPLE_RATE
#define DHA_MIC_SAMPLE_POINT AUDIO_FRAME_SIZE // 160 //64
#endif                                        /*DHA_NS_ENABLE*/
...
static const hearing_adda_param_t hearing_adda_param = {
    ...
    .mic_gain = 16, //12,                         // 12, //18,                         // 16, //12, //6, //12, //0, //4, //5,(ok1)      //15(big for RSZL mic)  , 12
    ...
    .dac_delay = (DHA_MIC_SAMPLE_POINT * TCFG_AD2DA_LOW_LATENCY_SAMPLE_RATE / DHA_MIC_SAMPLE_RATE) /
                     (TCFG_AD2DA_LOW_LATENCY_SAMPLE_RATE / 1000) +
                 5, // dac硬件混响延时， 单位ms 要大于 point_unit*2

#else
    .dac_delay = (DHA_MIC_SAMPLE_POINT * TCFG_AD2DA_LOW_LATENCY_SAMPLE_RATE / DHA_MIC_SAMPLE_RATE) /
                     (TCFG_AD2DA_LOW_LATENCY_SAMPLE_RATE / 1000) +
                 3, // dac硬件混响延时， 单位ms 要大于 point_unit*2
#endif
};
...
int hearing_aid_fitting_parse(u8 *data, u16 len)
{
    ...
    case DHA_FITTING_CMD_ADJUST:
        ...
        { // 开始验配，需要波单频音
            ...
            if ((channel == AUDIO_CH_L) && (adjust.channel == 0))
            { // 当前是左耳,并且数据是设置左耳时
            ...
            else if ((channel == AUDIO_CH_R) && (adjust.channel == 1))
            { // 当前是右耳，并且数据是设置右耳时
                for (int i = 0; i < DHA_FITTING_CHANNEL_MAX; i++)
                {
                    if (audio_mic_eq_tab[i].freq == adjust.freq)
...
static int hearing_aid_mic_en(u8 en, hearing_adda_param_t *param)
{
    ...
    if (en)
    {
        ...
        {
            printf("mic 0 open!\r\n");
            int ret = audio_adc_mic_open(&hdl->mic_ch, mic_ch, &adc_hdl);
            if (ret == 0)
                printf("mic 0 open ok!\r\n");
            else
                printf("mic 0 open failed!\r\n");
        ...
        {
            printf("mic 2 open!\r\n");
            int ret = audio_adc_mic2_open(&hdl->mic_ch, mic_ch, &adc_hdl);
            if (ret == 0)
                printf("mic 0 open ok!\r\n");
            else
                printf("mic 0 open failed!\r\n");
...
int hearing_output(hearing_aid_t *hdl, s16 *data, u16 len)
{
    ...
    if (hdl->channel == 1)
    {
        if (0)
        {
            ...
            float max_sample = 0;
            for (i = 0; i < (len >> 1); i++)
                if (fabs(mic_data[i]) > max_sample)
                    max_sample = fabs(mic_data[i]);
            if (max_sample > 10000.f)
            {
                float damp = 10000.f / max_sample;
                for (i = 0; i < (len >> 1); i++)
                    mic_data[i] *= damp;
            }
        }
    ...
    if (hdl->channel == 2)
    {
        ...
        for (u16 i = 0; i < (len >> 2); i++)
        {
            ...
            short left = mic_data[2 * i];
            short right = mic_data[2 * i + 1];
            mic_data[i] = right; //(short)(left*0.5f + right*0.5f);
...
#endif /*TCFG_AUDIO_DHA_FITTING_ENABLE*/
        {
            ...
            if (1)
            {
                ...
                else
                {
                    ...
                    //liurui add for two channels
                    // for (int i = 0; i < data_len / 2; i++)
                    // {
                    //     mic_data[i] = (hdl->tmp_buf[i] + hdl->tmp_buf_1[i])*0.5f;
                    //    // mic_data[2 * i + 1] = hdl->tmp_buf_1[i];
                    // }
                }
                ...
                else
                {
                    ...
                    // for (int i = 0; i < 64; i++)
                    // {
                    //     wav_tmp[2 * i] = near_buf[i * 2];
                    //     wav_tmp[2 * i + 1] = far_buf[i * 2];
                    // }
                    rbuf_write(ringbuf_dump, (char *)mic_data, 128);
                    // 唤醒一次运行任务处理数据
                    os_sem_set(&real_dump_sem, 0);
                    os_sem_post(&real_dump_sem);
#endif

#ifdef REAL_HEAR_DEMO
                    demo_cnt++;
                    // if (demo_cnt > 250 * 60 * 60)
                    if (demo_cnt > 2000000) // 1800000)  //1800000
                    {
                        for (int i = 0; i < data_len >> 1; i++)
                        {
                            mic_data[i] = 0;
...
#ifdef USE_CPULOAD_PRINT
    // {{
    os_system_info_reset(); // cpuload testing
    void task_info_add(TaskHandle_t task);
    task_info_add(os_task_get_handle("HearingAid")); // HEARING_AID_TASK_NAME
    // }}

    cpu_print_timer = sys_timer_add(NULL, cpu_load_schedule, 2500);
#endif

#ifdef REAL_DUMP_AUDIO_PCM_DATA

    ringbuf_dump = rbuf_create(64 * 5 * sizeof(int16_t)); // 25
    rbuf_set_mode(ringbuf_dump, RBUF_MODE_BLOCKING);
    rbuf_clear(ringbuf_dump);

    os_sem_create(&real_dump_sem, 0);
    err = task_create(dump_file_thread, NULL, "DUMP_FILE");
    if (err != OS_NO_ERR)
    {
        printf("******************************************************!");
        goto __err;
...
#define LIMITER_NOISE_GAIN ((int)(0.2 * (1 << 30))) /*(0~1)*2^30*/
    ...
    // // reset DAC volume: 20240910
    //     audio_dac_set_digital_vol(&dac_hdl, 0);
    //     audio_dac_set_analog_vol(&dac_hdl, 0);

#ifdef REALHEAR_ENABLE
    realhear = OpenRealHear(DHA_MIC_SAMPLE_RATE, DHA_MIC_SAMPLE_POINT);
#endif
    ...
    // sound_pcm_dev_start(&hdl->dac_ch,
    // TCFG_AD2DA_LOW_LATENCY_SAMPLE_RATE,
...
void audio_hearing_aid_sync_open(void)
{
    ...
    if (get_tws_sibling_connect_state())
    {
        if (tws_api_get_role() == TWS_ROLE_MASTER)
        {
            ...
            bt_tws_play_tone_at_same_time(SYNC_TONE_HEARING_AID_OPEN, 100);
        }
    }
    else
    {
...
int audio_hearing_aid_close(void)
{
    ...
    dump_is_running = 0;
    os_time_dly(1);
    rbuf_destroy(ringbuf_dump);
    ringbuf_dump = NULL;

    os_sem_del(&real_dump_sem, 0);
#endif
...
void hearing_aid_state_sync(u8 state)
{
    if (dha_schedule.state != state)
    {
        ...
        if ((dha_schedule.state == DHA_STATE_OPEN) && (dha_schedule.timer == 0))
        {
            dha_schedule.timer = sys_timer_add(NULL, hearing_aid_state_sync_deal, 500);
        }
    }
}
...
void audio_hearing_aid_resume(void)
{
    ...
    if ((dha_schedule.state == DHA_STATE_SUSPEND) && (dha_schedule.timer == 0))
    {
        dha_schedule.timer = sys_timer_add(NULL, hearing_aid_schedule, 500);
    }
#endif /*DHA_AND_MEDIA_MUTEX_ENABLE*/
}
